@php
    ...
    // Pending leave requests badge (scoped like the Leave Requests list).
    $pendingLeaves = 0;
    if ($user) {
        $lq = \App\Models\LeaveRequest::where('status', 'pending')
            ->whereHas('employee', fn ($q) => $q->where('role', 'employee'));
        if (! $user->isAdmin()) {
            $lq->where('user_id', $user->id);
        } elseif (! $user->isSuperAdmin()) {
            $lq->whereHas('employee', fn ($q) => $q->where('department_id', $user->department_id));
        }
        $pendingLeaves = $lq->count();
    }
@endphp
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-6 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('leave-requests.index')" :active="request()->routeIs('leave-requests.*')">
                        Leave Requests
                        @if ($pendingLeaves)
                            <span class="ms-1 inline-flex items-center justify-center min-w-[1.15rem] px-1.5 py-0.5 text-[10px] font-bold leading-none text-white bg-brand-600 rounded-full">{{ $pendingLeaves > 99 ? '99+' : $pendingLeaves }}</span>
                        @endif
                    </x-nav-link>
                    @if (isAdmin())
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('leave-requests.index')" :active="request()->routeIs('leave-requests.*')">
                Leave Requests @if ($pendingLeaves) <span class="ms-1 inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold text-white bg-brand-600 rounded-full">{{ $pendingLeaves > 99 ? '99+' : $pendingLeaves }}</span> @endif
